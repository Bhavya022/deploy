...
let registrationData = JSON.parse(localStorage.getItem("reg")) || [];
...
function deleteUser(event) {
    ...
    localStorage.setItem("reg", JSON.stringify(registrationData));
    row.remove();
}
...
function promptOTP(event) {
    let row = event.target.parentNode.parentNode;
    let uniqueId = row.children[0].innerHTML;
    let user = registrationData.find(user => user.id == uniqueId);
